...
Language translation: rongyao.li
Translation Author: 荣耀博客 - https://lirongyao.com/
*/
...
?>
<h3><?php printf( _n( 'There are currently %d locked out IP addresses.', 'There are currently %d locked out IP addresses.', count( $dalist ), 'loginlockdown' ), count( $dalist ) ); ?></h3>

<?php
	$num_lockedout = count($dalist);
	if( 0 == $num_lockedout ) {
		echo '<p>' . __( 'No IP blocks currently locked out.', 'loginlockdown' ) . '</p>';
	} else {
